...
# Rather than dynamically created paths just cd into the script directory
...
function link_file() 
{
	...
	msg_info "Linking source path ${src} to destination path ${path} ..."
...
function form_link_path()
{
	...
	case "$link_type" in 
	dotfile) dst="$HOME/.$(basename "${src%.dotfile}")";;
	symlink) dst="$HOME/$(basename "${src%.symlink}")";;
	*) echo "unknown symlink format ${link_type}"; exit 1;;
	esac 
...
function main() 
{
	...
	msg_info "Looking for files and directories to link within ${_SCRIPT_PATH} ..."
	find -H "$_SCRIPT_PATH" -maxdepth 3 \
		\( -name '*.symlink' -o -name '*.dotfile' \) \
		-not -path '*.git*' | while -r read src
	do
